...
// ROTA RAIZ - CORREÇÃO DO ERRO "Cannot GET /"
app.get('/', (req, res) => {
...
function getPlayersInZone(x, y, exceptId) {
  ...
  players.forEach((p, id) => {
    if (id !== exceptId && p.pos.x === x && p.pos.y === y && p.hp > 0) result.push(p);
  });
  return result;
}
...
io.on('connection', (socket) => {
  ...
  socket.on('createPlayer', ({ name, cls }) => {
    ...
    socket.broadcast.emit('playerJoined', { 
      id: player.id, name: player.name, cls: player.cls, lvl: player.lvl, pos: player.pos 
    });
...
function doAttack(attacker, defender) {
  ...
  attacker.socket.emit('combatLog', {
    ...
    dmg: result.dmg, crit: result.crit, defenderHp: defender.hp, defenderHpMax: defender.hpMax
  });
  defender.socket.emit('combatLog', {
    type: 'attacked', attacker: attacker.name, defender: defender.name,
    dmg: result.dmg, crit: result.crit, defenderHp: defender.hp, defenderHpMax: defender.hpMax
  });
...
function handleDeath(victim, killer) {
  ...
  if (Math.random() < 0.3) {
    ...
    addGroundItem(victim.pos.x, victim.pos.y, bn, 1); drops.push({ id: bn, qty: 1 });
  }
  ...
  killer.socket.emit('combatWon', {
    target: victim.name, xp: victim.lvl * 25, gold: victim.lvl * 15,
    drops: drops.map(d => ({ name: ITEMS[d.id]?.name || d.id, icon: ITEMS[d.id]?.icon || '📦' }))
  });
